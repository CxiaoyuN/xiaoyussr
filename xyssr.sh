...
Set_config_password(){
	...
	stty erase '^H' && read -p "(默认: xiaoyu):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="xiaoyu"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/CxiaoyuN/db-ssr/master/other/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/CxiaoyuN/db-ssr/master/other/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
